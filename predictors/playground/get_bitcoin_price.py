...
print(
    f"First btc price ts is: {first_btc_price_ts} and type is {type(first_btc_price_ts)}"
)
...
tweets = tweets.drop(
    columns=["timestamp"]
)  # index type: indexes.base.Index, index element type: str
tweets = tweets.set_index(
    pd.to_datetime(tweets.index)
)  # index type: indexes.datetimes.DatetimeIndex, index element type: timestamps.Timestamp
tweets = tweets.set_index(tweets.index.values.astype(np.int64) // 10 ** 9)
print(
    f"Type of whole index is: {type(tweets.index)}, type of one element is: {type(tweets.index[0])}"
)
tweets = tweets[
    tweets.index > first_btc_price_ts
]  # TODO: convert to first_btc_price_ts from Timestamp to datetime64 to be able to make this comparison
